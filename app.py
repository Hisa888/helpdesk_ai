...
def _read_xlsx_bytes(raw: bytes) -> pd.DataFrame:
    ...
    失敗した場合のみ従来のXML手動解析へフォールバックする。
    Excelのふりがな情報（rPh / phoneticPr）が混ざる不具合を避けるため、
    通常系では openpyxl を優先する。
    """
    from io import BytesIO
    ...
    try:
        ...
        if df is None:
            return pd.DataFrame()
        # 列名と値を文字列ベースで整える（既存の normalize_faq_columns に渡しやすくする）
