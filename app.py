...
def _read_xlsx_bytes(raw: bytes) -> pd.DataFrame:
    ...
    def _collect_visible_text(node) -> str:
        """
        Excel の sharedStrings / inlineStr から表示文字だけを取得する。
        ふりがな情報（rPh, phoneticPr など）は除外する。
        """
        if node is None:
            return ""
        ...
        def walk(el, blocked: bool = False):
            name = _local_name(getattr(el, "tag", ""))
            now_blocked = blocked or name in {"rPh", "phoneticPr"}
            if not now_blocked and name == "t":
                parts.append(el.text or "")
            for child in list(el):
                walk(child, now_blocked)
    ...
    with zipfile.ZipFile(BytesIO(raw)) as zf:
        ...
        if "xl/sharedStrings.xml" in zf.namelist():
            ...
            for si in root.findall("a:si", ns):
                shared.append(_collect_visible_text(si))
        ...
        for row in sheet.findall("a:sheetData/a:row", ns):
            ...
            for c in row.findall("a:c", ns):
                ...
                if cell_type == "s":
                    ...
                    if v is not None:
                        try:
                            si = int((v.text or "").strip())
                            value = shared[si] if 0 <= si < len(shared) else ""
                        except Exception:
                            value = ""
                elif cell_type == "inlineStr":
                    value = _collect_visible_text(c.find("a:is", ns))
                else:
                    v = c.find("a:v", ns)
