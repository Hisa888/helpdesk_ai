...
FEATURE_FLAGS_PATH = Path("feature_flags.json")
...
def default_feature_flags() -> dict:
    return {
        ...
        "hero_header": True,
        "kpi_cards": True,
        "sidebar_overview": True,
        "sidebar_expected_effect": True,
        "sidebar_usage": True,
        "sidebar_nohit_stats": True,
        "sidebar_simulator": True,
        "sidebar_charts": True,
        "sidebar_effect_report_pdf": True,
        "sidebar_log_download": True,
        "reference_faq": True,
        "suggested_questions": True,
        "nohit_extra_form": True,
        "admin_material_pdfs": True,
        "admin_faq_auto_generation": True,
    }


FEATURE_FLAG_LABELS = {
    "company_branding": "会社名・ロゴ表示",
    "contact_button": "導入相談ボタン",
    "hero_header": "ヒーローヘッダー",
    "kpi_cards": "KPIカード",
    "sidebar_overview": "サイドバー: このAIでできること",
    "sidebar_expected_effect": "サイドバー: 想定効果",
    "sidebar_usage": "サイドバー: 使い方",
    "sidebar_nohit_stats": "サイドバー: 問い合わせログ状況",
    "sidebar_simulator": "サイドバー: 削減時間シミュレーター",
    "sidebar_charts": "サイドバー: 見える化グラフ",
    "sidebar_effect_report_pdf": "サイドバー: 効果レポートPDF",
    "sidebar_log_download": "サイドバー: ログダウンロード",
    "reference_faq": "参照したFAQ（根拠）",
    "suggested_questions": "おすすめ質問ボタン",
    "nohit_extra_form": "該当なし時の追加情報フォーム",
    "admin_material_pdfs": "管理者向け資料PDF",
    "admin_faq_auto_generation": "管理者: FAQ自動生成",
}
...
def save_feature_flags(flags: dict) -> tuple[bool, dict]:
    clean = _sanitize_feature_flags(flags)
    try:
        FEATURE_FLAGS_PATH.write_text(json.dumps(clean, ensure_ascii=False, indent=2), encoding="utf-8")
        return True, clean
    except Exception:
        return False, clean
...
def feature_enabled(key: str) -> bool:
    return bool(st.session_state.get("feature_flags", FEATURE_FLAGS).get(key, default_feature_flags().get(key, True)))
...
def feature_flags_table_df() -> pd.DataFrame:
    flags = st.session_state.get("feature_flags", FEATURE_FLAGS)
    rows = []
    for key in default_feature_flags().keys():
        rows.append({
            "機能キー": key,
            "機能": FEATURE_FLAG_LABELS.get(key, key),
            "表示": "ON" if flags.get(key, False) else "OFF",
        })
...
with st.sidebar:
    ...
    if not st.session_state.is_admin:
        ...
        with st.expander("⚙️ 機能表示設定", expanded=False):
            st.caption("各機能の表示/非表示を切り替えられます。保存すると Streamlit の再起動後も保持されます。")
            current_flags = st.session_state.get("feature_flags", FEATURE_FLAGS).copy()
            edited_flags = {}
            for key in default_feature_flags().keys():
                edited_flags[key] = st.checkbox(
                    FEATURE_FLAG_LABELS.get(key, key),
                    value=bool(current_flags.get(key, default_feature_flags()[key])),
                    key=f"feature_flag_{key}",
                )

            st.dataframe(feature_flags_table_df(), width="stretch", hide_index=True)
            col_ff1, col_ff2 = st.columns(2)
            with col_ff1:
                if st.button("💾 機能設定を保存", width="stretch"):
                    ok, saved = save_feature_flags(edited_flags)
                    st.session_state.feature_flags = saved
                    if ok:
                        st.success("機能設定を保存しました。")
                    else:
                        st.error("機能設定の保存に失敗しました。")
            with col_ff2:
                if st.button("↺ 初期設定に戻す", width="stretch"):
                    ok, saved = save_feature_flags(default_feature_flags())
                    st.session_state.feature_flags = saved
                    if ok:
                        st.success("初期設定に戻しました。")
                        st.rerun()
                    else:
                        st.error("初期設定への復元に失敗しました。")
