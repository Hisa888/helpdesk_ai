...
''',
    ...
    def ensure_faq_index_loaded():
        ...
        # globals() 経由で取得し、未定義でも NameError にならないようにする
        ...
        globals()["X"] = X
        globals()["char_vectorizer"] = char_vectorizer
        globals()["X_char"] = X_char
        globals()["faq_embeddings"] = faq_embeddings
    ...
    def _domain_penalty(query_norm: str, row) -> float:
        """
        「PC本体の起動」と「Excel/Wordの起動」を混同しにくくするための軽い補正。
        FAQが無い領域を無理に当てないよう、ドメイン不一致を減点する。
        """
        try:
            ...
            # ユーザーが「PC/パソコン/Windows端末」を言っているのに、
            # FAQ側がアプリ起動系だけなら強めに下げる
            ...
            # 「Excel/Word/Chrome」など具体アプリ名が無いのにアプリ起動FAQへ寄りすぎるのを抑制
    ...
    def retrieve_faq(query: str):
        ...
        try:
            ...
            # ドメイン不一致補正:
            # 「パソコンが起動しません」で Excel/Word の起動FAQが上に来る現象を抑える
...
with st.expander("📂 FAQ管理（Excelダウンロード / アップロード）", expanded=False):
    ...
    if uploaded_faq is not None:
        try:
            raw_uploaded_df = normalize_faq_columns(read_faq_uploaded_file(uploaded_faq.name, uploaded_faq.getvalue()))
            improve_pack = build_faq_improvement_pack(raw_uploaded_df)
            incoming_df = improve_pack["faq_df"]
