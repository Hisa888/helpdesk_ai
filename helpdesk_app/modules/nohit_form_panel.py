...
def render_nohit_extra_form(*, st, update_nohit_record, info: dict | None = None, expanded: bool = True):
    ...
    with st.expander("📝 追加情報を記録（任意）", expanded=expanded):
        ...
        with st.form("nohit_extra_form", clear_on_submit=False):
            ...
            with c1:
                device = st.selectbox("端末", ["", "Windows", "Mac", "iPhone/iPad", "Android", "不明"], index=0, key="nohit_device")
            with c2:
                location = st.selectbox("利用場所", ["", "社内", "社外", "不明"], index=0, key="nohit_location")
            with c3:
                network = st.selectbox("ネットワーク", ["", "Wi-Fi", "有線", "VPN", "モバイル回線", "不明"], index=0, key="nohit_network")

            impact = st.selectbox("影響範囲", ["", "自分のみ", "他の人も", "不明"], index=0, key="nohit_impact")
            error_text = st.text_area("エラー内容（任意）", placeholder="例：0x80190001 / '資格情報が無効です' など", key="nohit_error_text")
