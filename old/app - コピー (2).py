...
def retrieve_faq(query: str):
    ...
    try:
        ...
        # まずは軽量検索だけで候補を絞る
        sims_base = (sims_word * 0.54) + (sims_char * 0.46)

        q_tokens = extract_search_tokens(query_norm)
        q_concepts = extract_concepts(query_norm)

        exact_bonus = (local_df["question_norm"] == query_norm).astype(float).to_numpy() * 0.28
        contains_bonus = local_df["question_norm"].apply(
            lambda x: 0.14 if query_norm and (query_norm in x or x in query_norm) else 0.0
        ).to_numpy()

        token_bonus = local_df["search_tokens"].apply(
            lambda toks: (0.24 * len(q_tokens & set(toks)) / max(1, len(q_tokens))) if q_tokens else 0.0
        ).to_numpy()

        concept_bonus = local_df["search_concepts"].apply(
            lambda cs: (0.24 * len(q_concepts & set(cs)) / max(1, len(q_concepts))) if q_concepts else 0.0
        ).to_numpy()

        prefix_bonus = local_df["question_norm"].apply(
            lambda x: 0.07 if query_norm and str(x).startswith(query_norm[: min(8, len(query_norm))]) else 0.0
        ).to_numpy()
        ...
        need_semantic = (
            ...
            and query_len >= 8
            and 0.24 <= preliminary_top <= 0.48
        )

        # 意味検索は「あいまいで軽量検索だけでは微妙」な時だけ
        if need_semantic:
            candidate_count = min(8, len(sims))
            ...
            if sims_sem is not None and len(sims_sem) == len(sims):
                ...
                boosted = sims.copy()
                boosted[candidate_idxs] = boosted[candidate_idxs] + (sem_arr[candidate_idxs] * 0.28)
                sims = boosted
